(ns fanout.social-graph
  "Social graph module materializing a load-balanced view of an account's followers.

   An account's followers are spread across many tasks of the module so that
   downstream fanout can be processed in parallel even if the social graph is
   extremely unbalanced (some accounts with millions of followers, average
   under a thousand).
  ...
  (:import [com.rpl.rama ModuleInstanceInfo]
           [fanout.protocol FollowAccount RemoveFollowAccount]))
