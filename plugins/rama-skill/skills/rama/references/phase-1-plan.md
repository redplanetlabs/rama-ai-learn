...
The `|hash`/`|all` indicators rule out some bad partitionings but not all — a partitioning can pass them and still waste work. **You MUST fill in the `## Partitioning efficiency` table in `PLAN.md`** (see the template in `references/artifact-plan.md`): for the dominant read, tabulate seeks/op and iterator-reads/op per **data category** (rows include the common/typical input, with frequency proportions summing to 1) at **N = 1, 4, and 16 tasks** (N = 1 is the single-task baseline), and compute the weighted sums Σ(proportion × seeks) and Σ(proportion × iterator-reads). If weighted seeks grow substantially from N = 1 to N = 16, the partitioning is wasting disk work on the frequent categories — you MUST redesign (consider creative `|direct` placement).
