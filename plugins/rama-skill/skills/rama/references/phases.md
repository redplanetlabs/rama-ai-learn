...
The decompose stage (after Phase 0) partitions the module into one or more subsystems in dependency order — see `phase-decompose.md`; each subsystem is a scoped subset of the full spec (the full spec stays the sole source of requirements), and the default is one subsystem covering the whole module. Each subsystem then runs its own plan and plan-validation; plan-validation classifies the subproblem easy/medium/hard (`phase-2-plan-validate.md`). An **easy** subproblem collapses the rest into a single `easy-build` session (`phase-easy-build.md`); **medium** and **hard** run the full gated phases 3–7. Each subsystem builds on the subsystems before it. On multi-subsystem builds the validation artifacts carry a `-<subsystem>` suffix; the module source and test namespaces are shared and accumulate, and each subsystem's final phase runs the full suite so earlier subsystems' tests stay green.
