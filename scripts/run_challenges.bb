...
(defn workflow-cmd
  ...
  The workflow (.claude/workflows/rama-challenge.js) drives all phases and
  their retry routing internally."
...
(defn parse-workflow-verdict
  ...
  on success and 'INCOMPLETE — <reason>' otherwise."
  [output]
...
(def ^:dynamic *outer-timeout-s*
  ...
  The workflow invocation binds this to min(default, *overall-timeout-s*)
  so the single workflow call can't exceed either cap."
  10800)

(def ^:dynamic *overall-timeout-s*
  "Hard cap on total wall-clock for one challenge run (seconds).
  Includes all phases, retries, lint, and test runs inside the workflow."
  (* 6 3600))
  ...
  (let [challenge-name (:name challenge)]
    (try
      ...
      (let [run-start-time (java.time.LocalDateTime/now)
            ...
            effective-timeout (min *outer-timeout-s* *overall-timeout-s*)
            _ (when *verbose*
                (println (format "  Workflow starting (budget: %ds)..." effective-timeout)))
